...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == "add") {
        ...
        if (!empty($customer_id) && !empty($passport_number) && !empty($name) && 
            !empty($surname) && !empty($email) && !empty($phone)) {
            
            // Check if email exists first (Solution 1)
            $check = $conn->prepare("SELECT id FROM travelers WHERE email = ?");
            ...
            if ($check->num_rows > 0) {
                ...
                // Try to insert with error handling (Solution 2)
                try {
...
<head>
    ...
    <link rel="stylesheet" href="/Travel_Agency/Presentation.Layer/UserInterface/css/travelers.css">
</head>
<body>
<section id="header">
